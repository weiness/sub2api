...
type SubscriptionPlan struct {
	...
	// BaseSoldCount holds the value of the "base_sold_count" field.
	BaseSoldCount int `json:"base_sold_count,omitempty"`
...
func (*SubscriptionPlan) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case subscriptionplan.FieldForSale, subscriptionplan.FieldRecommended:
			values[i] = new(sql.NullBool)
		case subscriptionplan.FieldPrice, subscriptionplan.FieldOriginalPrice:
			values[i] = new(sql.NullFloat64)
		case subscriptionplan.FieldID, subscriptionplan.FieldGroupID, subscriptionplan.FieldValidityDays, subscriptionplan.FieldSortOrder, subscriptionplan.FieldBaseSoldCount:
...
func (_m *SubscriptionPlan) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case subscriptionplan.FieldRecommended:
			if value, ok := values[i].(*sql.NullBool); !ok {
		...
		case subscriptionplan.FieldBaseSoldCount:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field base_sold_count", values[i])
			} else if value.Valid {
				_m.BaseSoldCount = int(value.Int64)
			}
...
func (_m *SubscriptionPlan) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("base_sold_count=")
	builder.WriteString(fmt.Sprintf("%v", _m.BaseSoldCount))
	builder.WriteString(", ")
