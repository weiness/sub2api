...
// The FAQFunc type is an adapter to allow the use of ordinary function as a Querier.
type FAQFunc func(context.Context, *ent.FAQQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f FAQFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.FAQQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.FAQQuery", q)
}

// The TraverseFAQ type is an adapter to allow the use of ordinary function as Traverser.
type TraverseFAQ func(context.Context, *ent.FAQQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseFAQ) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseFAQ) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.FAQQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.FAQQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.FAQQuery:
		return &query[*ent.FAQQuery, predicate.FAQ, faq.OrderOption]{typ: ent.TypeFAQ, tq: q}, nil
	case *ent.GroupQuery:
