...
func TestToModelPlazaGroupDTO_UserRateAndFieldWhitelist(t *testing.T) {
	g := service.PlazaGroup{
		...
		AllowImageGeneration: true, ImageRateIndependent: true, ImageRateMultiplier: 1.2,
		ImagePrice1K: testPtr(0.04),
	...
	require.Equal(t, []any{"text", "image"}, model["modalities"])
	require.Equal(t, []any{"text"}, model["output_modalities"])
	require.Equal(t, []any{"function_calling"}, model["capabilities"])
