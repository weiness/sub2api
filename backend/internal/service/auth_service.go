...
var (
	...
	ErrRegistrationIPLimitExceeded = infraerrors.TooManyRequests("REGISTRATION_IP_LIMIT_EXCEEDED", "too many accounts have been registered from this IP; please try again later or contact the administrator")
)
...
func (s *AuthService) BindPhone(ctx context.Context, userID int64, phone, code string) (*User, error) {
	...
	if err := s.userRepo.Update(ctx, user, UserUpdateFields{Phone: true}); err != nil {
		return nil, err
	}
	return user, nil
}
...
func (s *AuthService) LoginOrRegisterOAuth(ctx context.Context, email, username string) (string, *User, error) {
	...
	if err != nil {
		if errors.Is(err, ErrUserNotFound) {
			...
			s.applyRegistrationIPPolicy(ctx, newUser)
