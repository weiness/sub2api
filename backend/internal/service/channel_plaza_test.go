...
func TestListPlazaGroups_GroupCentricAggregation(t *testing.T) {
	...
	groups := []Group{
		{ID: 10, Name: "g-main", Description: "desc", Platform: "anthropic", RateMultiplier: 1, AllowImageGeneration: true, ImageRateIndependent: true, ImageRateMultiplier: 1.2, ImagePrice1K: testPtrFloat64(0.04)},
	...
	require.True(t, out[0].AllowImageGeneration)
	require.True(t, out[0].ImageRateIndependent)
	require.InDelta(t, 1.2, out[0].ImageRateMultiplier, 1e-12)
	require.InDelta(t, 0.04, *out[0].ImagePrice1K, 1e-12)
...
func TestListPlazaGroups_OfficialPricingFill(t *testing.T) {
	...
	require.Equal(t, []string{"text", "image"}, byName["claude-sonnet"].Modalities)
	require.Equal(t, []string{"text"}, byName["claude-sonnet"].OutputModalities)
	require.Equal(t, []string{"function_calling", "reasoning"}, byName["claude-sonnet"].Capabilities)
