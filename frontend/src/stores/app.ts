...
export const useAppStore = defineStore('app', () => {
  ...
  const updateCheckEnabled = ref<boolean>(true)
  const onlineUpdateEnabled = ref<boolean>(true)
  ...
  async function fetchVersion(force = false): Promise<VersionInfo | null> {
    ...
    if (versionLoaded.value && !force) {
      return {
        ...
        cached: true,
        update_check_enabled: updateCheckEnabled.value,
        online_update_enabled: onlineUpdateEnabled.value
      }
    }
    ...
    try {
      ...
      updateCheckEnabled.value = data.update_check_enabled ?? true
      onlineUpdateEnabled.value = data.online_update_enabled ?? true
