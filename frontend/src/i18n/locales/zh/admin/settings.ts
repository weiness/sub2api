    ...
    settings: {
      ...
      features: {
        ...
        availableChannels: {
          ...
          channelStatusEnabled: '启用渠道状态',
          channelStatusEnabledHint: '关闭后用户端侧边栏入口隐藏，接口返回空数组。',
        },
        modelPlaza: {
          title: '模型广场',
          description: '以分组为单位向访客展示可用模型与价格的公开页面。默认关闭。',
          enabled: '启用模型广场',
          enabledHint: '开启后个人菜单显示入口，页面可通过 /model-plaza 独立访问。',
          requireAuth: '需要登录才可访问',
          requireAuthHint: '开启后未登录访问将跳转登录页；关闭则公开展示模型目录，但分组与倍率仍需登录后查看。',
      ...
      site: {
        ...
        homeTitle: '首页主标题',
        homeTitlePlaceholder: '把预算花在',
        homeTitleHint: '营销首页主标题的普通文本部分',
        homeTitleHighlight: '首页强调标题',
        homeTitleHighlightPlaceholder: '真正好用的模型。',
        homeTitleHighlightHint: '营销首页主标题中使用品牌色强调的部分',
        apiBaseUrl: 'API 端点地址',
